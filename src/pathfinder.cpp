...
void PathFinder::drawLine(CvPoint A, CvPoint B, IplImage *img) {
	...
	for (int i = 0; i <= d; i++) {
		drawPoint(
				cvPoint(A.x + i * (B.x - A.x) / d, A.y + i * (B.y - A.y) / d),
				img);
	}
...
PathFinderAllRed::PathFinderAllRed(Model *m) :
	PathFinder(m) {
}
...
void PathFinderMaxSquare::drawPath(IplImage *frame, IplImage *desktop) {
	...
	for (y = 1; y < H; y++)
		for (x = 3; x < W; x += 3) {
			...
			int B = (unsigned char) frame->imageData[y * W + x];
			if (isLightPen(R, G, B)) {
				maxSquare[y][x / 3] = min(maxSquare[y - 1][x / 3], min(
						maxSquare[y][x / 3 - 1], maxSquare[y - 1][x / 3 - 1]))
						+ 1;
...
PathFinderMaxSquare::PathFinderMaxSquare(Model *m) :
	PathFinder(m) {
...
void PathFinderFitLine::drawPath(IplImage *frame, IplImage *desktop) {
	...
	for (y = min(model->calibrator->calibrationData.vertex[0].y,
			model->calibrator->calibrationData.vertex[1].y); y < max(
			model->calibrator->calibrationData.vertex[2].y,
			model->calibrator->calibrationData.vertex[3].y); y++)
		for (x = 3 * min(model->calibrator->calibrationData.vertex[0].x,
				model->calibrator->calibrationData.vertex[1].x); x < 3 * max(
				model->calibrator->calibrationData.vertex[2].x,
				model->calibrator->calibrationData.vertex[3].x); x += 3) {
			int R = (unsigned char) frame->imageData[y * W + x + 2];
			int G = (unsigned char) frame->imageData[y * W + x + 1];
			int B = (unsigned char) frame->imageData[y * W + x];
			if (isLightPen(R, G, B)) {
...
PathFinderFitLine::PathFinderFitLine(Model *m) :
	PathFinder(m) {
	storage = cvCreateMemStorage(0);
	point_seq = cvCreateSeq(CV_32FC2, sizeof(CvSeq), sizeof(CvPoint2D32f),
			storage);
}
