...
IplImage* Calibrator::DetectAndDrawQuads(IplImage* img) {
	...
	IplImage *resizedRet = cvCreateImage(cvSize(model->blackBoardWidth,
			model->blackBoardHeight), 8, 1);
...
int Calibrator::calibrate() {
	...
	for (int i = 0; i < 4; i++) {
		...
		cerr << i << ": " << calibrationData.vertex[i].x << " "
				<< calibrationData.vertex[i].y << endl;
	}
